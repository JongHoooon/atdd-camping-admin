...
시작하기 전에 `.claude/gate/armed` 파일이 있으면 지운다. 이 스킬은 지금부터 실패하는
인수 테스트를 새로 쓴다 — Stop 훅이 그 실패를 통과 실패로 오인해 턴을 막지 않도록
무장을 해제한다.
...
`docs/plan.md`의 "이 저장소의 실행 방법" 중 "특정 테스트 클래스만 실행" 명령으로 새/수정된
클래스만 실행한다.

결과를 못 읽으면 라벨과 상관없이 통과로 치지 않는다(`docs/principles.md` "확인이 안 되면
통과가 아니다").

라벨별로 기대하는 결과가 다르다:

- `버그` 라벨에서 나온 테스트는 **지금 실패해야 한다.** 통과하면 지금 동작(버그)을 겨눈 초안이라는
  뜻이므로(`docs/plan.md` "넘어가기 전에" 원칙), 무엇을 assert했는지 다시 보고 원하는 동작(status
  code, 메시지)을 assert하도록 고친 뒤 다시 실행한다.
- `정상` / `정상, 회귀 방지` 라벨에서 나온 테스트는 **지금 통과해야 한다** (회귀 방지 목적).
